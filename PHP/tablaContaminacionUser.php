...
//          $html .= '<td>' . $row['fotoAgua'] . '</td>'; //ESTE CAMPO NO SE MUESTRA EN LA BASE DE DATOS, ES POR ESO QUE SE QUITA (POR AHORA) PARA QUE SE MUESTREN LOS DATOS
            ...
            $html .= '<td class="edit-table"><a href="contaminacionUserPage.php?contaminationEditedField='.$row['codigoAgua'].'" name="editarCampo"> Edit </a></td>';
            $html .= '<td  class="eliminate-table"><a href="contaminacionUserPage.php?contaminationDeteledField='.$row['codigoAgua'].'" name="eliminarCampo"> Delete </a></td>';
        $html .= '</tr>';
    }
} else {
    $html .= '<tr>';
